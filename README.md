...
- 🎨 **Bubble Tea & Lip Gloss Aesthetic**: Vibrant pastel palette, tabbed layout, frequency visual meters, status pills, and keyboard navigation.
...
2. Export your key in your environment or enter it in the **Settings [4]** tab:
...
| Key | Action |
| :--- | :--- |
| `Tab` / `Shift+Tab` | Switch tabs (*Explore* / *Review* / *Network* / *Settings*) |
| `1` - `4` | Switch tab or rate flashcard recall (*1: Again, 2: Hard, 3: Good, 4: Easy*) |
| `↑` / `↓` | Select word from sidebar list |
...
```
pkg/
├── api/       # Wordnik API client, rate-limit header parser, disk cache
├── seed/      # Rich offline seed dataset of uncommon/useful vocabulary
├── srs/       # SuperMemo SM-2 spaced repetition algorithm
├── storage/   # User config & SRS progress database (~/.config/wordy/)
└── tui/       # Bubble Tea model, Lip Gloss styles, and component views
```
